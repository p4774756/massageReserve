...
- 首頁預約頁頂部為 LED 跑馬燈一則：`siteSettings/marqueeLed`（`text`、`enabled`，另可選 `speed` 像素／秒，後台以拉霸調整）。舊的 `siteSettings/announcement` 與頂部文字 `marqueeText` 已不再使用。
